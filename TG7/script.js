    ...
    if (container2Content) {
        ...
        uniqueFilenames.forEach((filename, index) => {
            ...
            fileDiv.addEventListener('click', function() {
                fetch('http://localhost:5500/get_file_content', {
                    method: 'POST',
...
async function sendMessageToOllama(message) {
    ...
    try {
        const response = await fetch('http://localhost:5500/rag_query', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        // Add Ollama/Flask response with filenames (show error if present)
        chatHistory.push({ 
            role: 'assistant', 
            content: data.llm_answer || data.response || 'No response from Ollama',
            filenames: data.referenced_files || data.filenames || []
        });
...
function setupVersionControlFilter() {
    const versionFilterInput = document.getElementById('version-filter-input');
    const clearFilterBtn = document.getElementById('clear-version-filter');
    
    if (versionFilterInput && clearFilterBtn) {
        versionFilterInput.addEventListener('input', (e) => {
            const filterText = e.target.value.trim().toLowerCase();
            filterVersionControlFiles(filterText);
        });
        
        clearFilterBtn.addEventListener('click', () => {
            versionFilterInput.value = '';
            filterVersionControlFiles('');
        });
    }
}

function filterVersionControlFiles(filterText) {
    const rows = document.querySelectorAll('.version-file-row');
    let visibleCount = 0;
    ...
    container2.innerHTML = `
        ...
        ">
            ...
            <div style="display: flex; gap: 10px; align-items: center;">
                <input type="text" id="tag-filter-input" placeholder="Filter..." style="
                    background: white; 
                ...
                <button id="clear-tag-filter" style="
                    background: linear-gradient(to right, #111825, #294572); 
                    color: white; 
                    border: 1px solid #ccc; 
                    padding: 6px 10px; 
                    border-radius: 3px; 
...
function setupTagsTable() {
    ...
    container2.innerHTML = `
        ...
        ">
            ...
            <div style="display: flex; gap: 10px; align-items: center;">
                <input type="text" id="tags-filter-input" placeholder="Filter..." style="
                    background: white; 
                    border: 1px solid #ccc; 
                    color: black; 
                    padding: 6px 10px; 
...
function setupVersionControlTable() {
    ...
    container2.innerHTML = `
        ...
        ">
            ...
            <div style="display: flex; gap: 10px; align-items: center;">
                <input type="text" id="version-filter-input" placeholder="Filter..." style="
                    background: white; 
                    border: 1px solid #ccc; 
                    color: black; 
                    padding: 6px 10px; 
                    ...
                    min-width: 120px;
                ">
                <button id="clear-version-filter" style="
                    background: linear-gradient(to right, #111825, #294572); 
                    color: white; 
                    border: 1px solid #ccc; 
                    padding: 6px 10px; 
    ...
    // Show universal back button and setup filter
    setTimeout(() => {
        setupVersionControlFilter();
        showUniversalBackButton();
    }, 100);
}
        ...
        <tr class="recent-file-row" data-filename="${escapeHtml(file.name)}">
            ...
            <td class="actions-cell">
                <div class="button-group">
                    ...
                    <button onclick="tagFileByName('${escapeHtml(file.name)}')" class="tag-btn">🏷️ Tag</button>
        ...
        <tr class="version-file-row" data-filename="${escapeHtml(fileName)}">
            <td>
                <div class="file-info">
                    ...
                    <div>
                        ...
                        <div class="file-details">${fileExtension.toUpperCase()} File</div>
                    </div>
                </div>
            </td>
            <td>
            ...
            <td class="actions-cell">
                <div class="button-group">
                    ...
                    <button onclick="tagFileByName('${escapeHtml(fileName)}')" class="tag-btn">🏷️ Tag</button>
...
function transitionToRecentsView() {
    ...
    setTimeout(() => {
        ...
        // Add scale-in animation to the table
        const table = container2.querySelector('.recent-files-table');
        if (table) {
            table.style.transform = 'scale(0.8)';
            table.style.opacity = '0';
            table.style.transition = 'all 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)';
...
function returnToHomeScreen() {
    ...
    setTimeout(() => {
        ...
        // Reset container styles properly
        // For container1 - remove all inline styles to restore original behavior
        container1.style.cssText = '';
        
        // For container2 - reset to original state but preserve original dimensions
        container2.style.cssText = '';
        
        // Ensure container1 is in its default state
        container1.style.display = '';  // Let CSS handle display
        container1.style.opacity = '';  // Let CSS handle opacity
        container1.style.transform = ''; // Let CSS handle transform
        
        // Ensure container2 is properly hidden and positioned
        container2.style.display = '';  // Let CSS handle display when hidden class is present
        container2.style.opacity = '';  // Let CSS handle opacity
        container2.style.transform = ''; // Let CSS handle transform
        ...
        // Force a final cleanup to ensure everything is reset
        setTimeout(() => {
            // Ensure body class is cleared
            body.classList.remove('containers-expanded');
            
            // Ensure container2 is properly hidden
            container2.classList.add('hidden');
            
            // Remove any remaining inline styles that might interfere
...
function transitionToTagsView() {
    ...
    setTimeout(() => {
        ...
        // Add scale-in animation to the table
        const table = container2.querySelector('.tags-table');
        if (table) {
            table.style.transform = 'scale(0.8)';
            table.style.opacity = '0';
            table.style.transition = 'all 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)';
...
function transitionToVersionControlView() {
    ...
    setTimeout(() => {
        ...
        // Add scale-in animation to the table
        const table = container2.querySelector('.version-control-table');
        if (table) {
            table.style.transform = 'scale(0.8)';
            table.style.opacity = '0';
            table.style.transition = 'all 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)';
...
// Tag file by name function with priority dropdown
function tagFileByName(fileName) {
    // Create modal overlay (styled like edit overlay)
    const modal = document.createElement('div');
    modal.id = 'tag-priority-modal';
    modal.style.cssText = `
        position: fixed;
        top: 0;
        left: 0;
    ...
    modalContent.innerHTML = `
        ...
        ">
            <div>
                <h2 style="margin: 0; color: #ffffff; font-size: 22px;">🏷️ Set Priority Tag</h2>
                <p style="margin: 5px 0 0 0; color: #888; font-size: 14px;">Choose priority level for "${escapeHtml(fileName)}"</p>
            </div>
            <div>
                <button id="cancel-tag-btn" style="
                    background: #d73a49; 
                    color: white; 
                    border: none; 
                    padding: 8px 12px; 
                    ...
                    cursor: pointer;
                    font-size: 12px;
                " onmouseover="this.style.background='#c82333'" onmouseout="this.style.background='#d73a49'">✕</button>
            </div>
        </div>
        <div class="tag-content" style="
            flex: 1;
            display: flex;
            flex-direction: column;
            justify-content: center;
        ...
        ">
            <div style="display: flex; flex-direction: column; gap: 20px; max-width: 600px; width: 100%;">
                ...
                <button class="priority-btn" data-priority="Medium" style="
                    ...
                    justify-content: center;
                    gap: 12px;
                " onmouseover="this.style.background='#e0a800'; this.style.borderColor='#e0a800';" 
    ...
    priorityButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            applyPriorityTag(fileName, priority);
    ...
    document.addEventListener('keydown', function escapeHandler(e) {
        if (e.key === 'Escape') {
            if (document.getElementById('tag-priority-modal')) {
                document.body.removeChild(modal);
            }
...
// Apply priority tag function
function applyPriorityTag(fileName, priority) {
    // Get existing tags and remove any existing priority tags
    const existingTags = fileTags.get(fileName) || [];
    const nonPriorityTags = existingTags.filter(tag => 
        tag !== 'High' && tag !== 'Medium' && tag !== 'Low'
    );
    
    // Add the new priority tag
    const allTags = [...nonPriorityTags, priority];
    
    fileTags.set(fileName, allTags);
    
    // Update displays and persist state
    updateRecentFilesDisplay();
    updateTagsIndicator();
    persistState();
    
    // Refresh the current view if we're in tags view
    const tagsSidebarActive = document.querySelector('.sidebar-item[data-section="tags"].active');
    if (tagsSidebarActive) {
        setupTagsTable();
    }
    
    // Refresh version control view if that's active
    const versionSidebarActive = document.querySelector('.sidebar-item[data-section="version"].active');
    if (versionSidebarActive) {
        setupVersionControlTable();
    }
}
