...
# Simple chat endpoint that talks directly to Ollama (no RAG)
...
def chat():
    try:
        ...
        if not message or not isinstance(message, str):
            return jsonify({'error': 'Message is missing or invalid.'}), 400
        
        # Send message directly to Ollama
        response = requests.post(
            "http://localhost:11434/api/generate",
            json={
                "model": "llama3",  # Change this to your installed model if needed
                "prompt": message,
                "stream": False
            },
        ...
        return jsonify({'error': 'Could not connect to Ollama. Make sure Ollama is running on localhost:11434'}), 500
    except requests.exceptions.Timeout:
        return jsonify({'error': 'Request timed out. Ollama might be busy.'}), 500
    except Exception as e:
...
def get_file_content():
    try:
        ...
        # Security check - ensure the file is in our allowed directory
        ...
        try:
            ...
            try:
                ...
                return jsonify({'error': 'Could not read file - unsupported encoding'}), 400
